...
const Login = () => {
  ...
  const { loginEmailAndPassword, user, logOut, passwordReset, error } =
      ...
      <div className="w-[350px] min-h-[550px] bg-gradient-to-r hover:-translate-y-1 duration-300  hover:shadow-lg hover:shadow-red-500 from-indigo-500 via-purple-500 to-pink-500 font-bold text-white rounded-lg p-1">
        <div className="bg-black min-h-[546px] w-[346] rounded-lg relative">
          ...
          <h1 className="text-2xl text-slate-400 tracking-wider text-center pt-5">
            {isResetInput ? "Reset Password" : "Please Login"}
          </h1>
          <div
            onKeyDown={(e) => {
              if (e.key == "Enter") {
          ...
          >
            ...
            {isResetInput ? (
              <button
                onClick={() => {
                  if (passResetEmail.length >= 10) {
                    handlePassReset();
                    setIsResetInput(false);
                  } else toast.error("Please enter your password reset email");
                }}
              ...
              <button
                onClick={handleLogin}
                className="w-full text-md text-center h-8  bg-[#F221FF] text-slate-300 mt-5 rounded-full"
              >
                Login
              </button>
            )}
