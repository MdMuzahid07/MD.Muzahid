...
const DashboardUpdateCourse = () => {
  const { courseId } = useParams();
  const [selectedDate, setSelectedDate] = useState(null);
  const [selectedSkills, setSelectedSkills] = useState([]);
  const [updateCourse, { data, error, isLoading }] = useUpdateCourseMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await updateCourse({ id: courseId, data: updateData });
    } catch (error) {
  ...
  return (
    <section className={`${styles.dashboardPageCardBgWhiteOpacity} mb-20`}>
      <h1 className={`${addProjectStyles.headingText} mb-14`}>Update Course</h1>
...
export default DashboardUpdateCourse;
